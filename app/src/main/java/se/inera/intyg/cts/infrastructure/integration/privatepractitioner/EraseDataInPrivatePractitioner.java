...
public class EraseDataInPrivatePractitioner implements EraseDataInService {
  ...
  private static final Logger LOG = LoggerFactory.getLogger(EraseDataInPrivatePractitioner.class);
  private static final ServiceId SERVICE_ID = new ServiceId("pps");
  ...
  @Override
  @PerformanceLogging(eventAction = "erase-in-pps", eventType = EVENT_TYPE_DELETION)
  public void erase(Termination termination) throws EraseException {
    try {
      webClient
          ...
          .uri(
              UriComponentsBuilder.fromUriString(baseUrl)
                  .path("/{careProvider}")
                  .buildAndExpand(termination.careProvider().hsaId().id())
                  .toUri())
      ...
      LOG.error("Error calling private practitioner service to delete care provider.", ex);
